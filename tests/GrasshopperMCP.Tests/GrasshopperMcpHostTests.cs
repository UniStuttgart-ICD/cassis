...
namespace GrasshopperMCP.Tests;

[TestFixture]
public class GrasshopperMcpHostTests
{
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new GrasshopperMcpHost(null!));
    }

    [Test]
    public void Constructor_Throws_For_Empty_Prefix()
    {
        Assert.Throws<ArgumentException>(() => new GrasshopperMcpHost(""));
    }

    [Test]
    public void Constructor_Rejects_NonLoopback_Prefix()
    {
        Assert.Throws<ArgumentException>(() => new GrasshopperMcpHost("http://0.0.0.0:3001/test/"));
    }

    [Test]
    public void Constructor_Should_Initialize_With_Valid_Prefix()
    {
        var host = new GrasshopperMcpHost(_testPrefix);
        Assert.NotNull(host);
    }
    ...
    {
        await using var host = new GrasshopperMcpHost(_testPrefix);
    ...
    private static async Task<string> GetToolsListContent(GrasshopperMcpHost host, string prefix, HttpClient client)
    {
    ...
    public async Task Regression_AssemblyRename_ToolsListReturnsAtLeastOneTool()
    {
        ...
        await using var host = new GrasshopperMcpHost(prefix);
    ...
    public async Task Regression_RestrictedEnabledTools_OnlyExposesAllowedTools()
    {
        ...
        await using var host = new GrasshopperMcpHost(prefix, enabledTools: new[] { "getsystemhealth" });
        using var client = new HttpClient();
